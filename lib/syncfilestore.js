...
SyncFileStore.prototype.path = function (key) {
    ...
    var parts = hash.match(/.{1,2}/g).slice(0, 2).join(this.pathHelper.sep);
    return [this.directory, parts, hash].join(this.pathHelper.sep);
};
...
SyncFileStore.prototype.flush = function(callback) {
    try{
        rmDir(this.directory, this.fs, this.pathHelper);
        return callback(null, true);
    }catch(e){
        return callback(e, null);
    }
...
function rmDir(dirPath , fs , path) {
